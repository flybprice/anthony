...
def login1(emp_number='034756', emp_password='Peteyc44!', emp_security_answer='NULL'):
...
def click():
    ...
    while x <= 11:
        pyautogui.click(a, b)
        pyautogui.click(1370, 605)  # EO LIST
...
seconds_click = str('57')  # start clicking 3 seconds before avail time

schedule.every().day.at(f"{hours}:{mins}:{seconds_login}").do(login1)
schedule.every().day.at(f"{hours}:{mins}:{seconds_click}").do(click)
